...
void splitParameters(at::Tensor &x_min, at::Tensor &x_max, at::Tensor &y_min,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      x_min.scalar_type(), "gpu::splitParameters", ([&] {
        splitParametersKernel<<<numBlocks, BLOCK_SIZE, 0,
...
void splitParametersUpdateGradInput(at::Tensor &x_min, at::Tensor &x_max,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      x_min.scalar_type(), "gpu::splitParametersUpdateGradInput", ([&] {
...
void splitParametersAccGradParameters(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      x_min.scalar_type(), "gpu::splitParametersAccGradParams", ([&] {
...
template <typename scalar_t>
__global__ void clipParametersKernel(scalar_t *__restrict__ paramMin,
                                     scalar_t *__restrict__ paramMax,
                                     const double inverseReparam,
...
void clipParameters(at::Tensor &paramMin, at::Tensor &paramMax,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      paramMin.scalar_type(), "gpu::clipParameters", ([&] {
        clipParametersKernel<<<numBlocks, BLOCK_SIZE, 0,
                               at::cuda::getCurrentCUDAStream()>>>(
...
__global__ void computeAreaKernel(scalar_t *__restrict__ x_min,
                                  scalar_t *__restrict__ x_max,
                                  scalar_t *__restrict__ y_min,
                                  scalar_t *__restrict__ y_max,
                                  scalar_t *__restrict__ retval,
                                  const int nElements) {
  const int idx = BLOCK_SIZE * blockIdx.x + threadIdx.x;
...
at::Tensor computeArea(at::Tensor x_min, at::Tensor x_max, at::Tensor y_min,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      x_min.scalar_type(), "gpu::computeArea", ([&] {
        if (needXDeriv) {
          if (needYDeriv) {
            computeAreaKernel<true, true><<<numBlocks, BLOCK_SIZE, 0,
                                            at::cuda::getCurrentCUDAStream()>>>(
                x_min.data_ptr<scalar_t>(), x_max.data_ptr<scalar_t>(),
                y_min.data_ptr<scalar_t>(), y_max.data_ptr<scalar_t>(),
          ...
          if (needYDeriv) {
            ...
            THError(
                "computeArea called with needXDeriv == needYDeriv == false");
          }
        }
      }));
  THCudaCheck(cudaGetLastError());
