...
__global__ void boxConvUpdateOutputKernel(CudaAcsr<scalar_t, 3> inputInt,
                                          CudaAcsr<scalar_t, 5> output,
                                          const int32_t *__restrict__ xMinInt,
                                          const int32_t *__restrict__ xMaxInt,
                                          const int32_t *__restrict__ yMinInt,
                                          const int32_t *__restrict__ yMaxInt,
...
__global__ void boxConvUpdateOutputKernel(
  ...
  if (x < h and y < w) {
    ...
    // -- main area
    outValue = inputIntPlane[b][r] - inputIntPlane[t][r] - inputIntPlane[b][l] +
               inputIntPlane[t][l];

    // -- xMax border
    outValue += (inputIntPlane[bAdv][r] - inputIntPlane[b][r] -
                 inputIntPlane[bAdv][l] + inputIntPlane[b][l]) *
                xMaxCurrFrac;

    // -- yMax border
    outValue += (inputIntPlane[b][rAdv] - inputIntPlane[b][r] -
                 inputIntPlane[t][rAdv] + inputIntPlane[t][r]) *
                yMaxCurrFrac;

    // -- xMin border
    outValue += (inputIntPlane[t][r] - inputIntPlane[tAdv][r] -
                 inputIntPlane[t][l] + inputIntPlane[tAdv][l]) *
                xMinCurrFrac;

    // -- yMin border
    outValue += (inputIntPlane[b][l] - inputIntPlane[b][lAdv] -
                 inputIntPlane[t][l] + inputIntPlane[t][lAdv]) *
                yMinCurrFrac;
    ...
    if (not((x + xMaxCurr >= h) | (y + yMaxCurr >= w) | (x + xMaxCurr < 0) |
            (y + yMaxCurr < 0))) {
      outValue += xMaxCurrFrac * yMaxCurrFrac *
                  (inputIntPlane[b + 1][r + 1] - inputIntPlane[b][r + 1] -
                   inputIntPlane[b + 1][r] + inputIntPlane[b][r]);
    }

    if (not((x + xMinCurr > h) | (y + yMaxCurr >= w) | (x + xMinCurr <= 0) |
            (y + yMaxCurr < 0))) {
      outValue += xMinCurrFrac * yMaxCurrFrac *
                  (inputIntPlane[t][r + 1] - inputIntPlane[t - 1][r + 1] -
                   inputIntPlane[t][r] + inputIntPlane[t - 1][r]);
    }

    if (not((x + xMaxCurr >= h) | (y + yMinCurr > w) | (x + xMaxCurr < 0) |
            (y + yMinCurr <= 0))) {
      outValue += xMaxCurrFrac * yMinCurrFrac *
                  (inputIntPlane[b + 1][l] - inputIntPlane[b][l] -
                   inputIntPlane[b + 1][l - 1] + inputIntPlane[b][l - 1]);
    }

    if (not((x + xMinCurr > h) | (y + yMinCurr > w) | (x + xMinCurr <= 0) |
            (y + yMinCurr <= 0))) {
      outValue += xMinCurrFrac * yMinCurrFrac *
                  (inputIntPlane[t][l] - inputIntPlane[t - 1][l] -
                   inputIntPlane[t][l - 1] + inputIntPlane[t - 1][l - 1]);
    }
...
void boxConvUpdateOutput(at::Tensor &xMinInt, at::Tensor &xMaxInt,
  ...
  const dim3 gridSize((totalOutputChannels + blockSize.x - 1) / blockSize.x,
                      (w + blockSize.y - 1) / blockSize.y,
                      (h + blockSize.z - 1) / blockSize.z);

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      output.scalar_type(), "gpu::boxConvUpdateOutput", ([&] {
        ...
        if (exact) {
          boxConvUpdateOutputKernel<normalize>
              <<<gridSize, blockSize, 0, at::cuda::getCurrentCUDAStream()>>>(
                  inputIntAcsr, outputAcsr, xMinInt.data_ptr<int32_t>(),
                  xMaxInt.data_ptr<int32_t>(), yMinInt.data_ptr<int32_t>(),
                  yMaxInt.data_ptr<int32_t>(), xMinFrac.data_ptr<scalar_t>(),
                  xMaxFrac.data_ptr<scalar_t>(), yMinFrac.data_ptr<scalar_t>(),
                  yMaxFrac.data_ptr<scalar_t>(),
