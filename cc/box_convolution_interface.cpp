/* Functions actually called from Python. Registered in torch module in
 * `bind.cpp` */
...
at::Tensor box_convolution_forward(at::Tensor input_integrated,
  ...
  input_integrated =
      input_integrated.contiguous();  // TODO support noncontiguous too
  TORCH_CHECK(input_integrated.dim() == 4,
              "BoxConv2d: input must have 4 dimensions");
  TORCH_CHECK(x_min.dim() == 2 and x_max.dim() == 2 and y_min.dim() == 2 and
                  y_max.dim() == 2,
              "BoxConv2d: all parameters must have 2 dimensions");
  TORCH_CHECK(x_min.size(0) == x_max.size(0) and
                  x_min.size(0) == y_min.size(0) and
                  x_min.size(0) == y_max.size(0) and
                  x_min.size(0) == input_integrated.size(1),
              "BoxConv2d: all parameters must have as many rows as there are "
              "input channels");
  TORCH_CHECK(x_min.size(1) == x_max.size(1) and
                  x_min.size(1) == y_min.size(1) and
  ...
  // TODO how to force is_variable(false)? How to set is_variable on an existing
  // tensor?
  auto intOptions =
      x_min.options().is_variable(true).dtype(at::ScalarType::Int);
  ...
  if (x_min.is_cuda()) {
    gpu::splitParameters(x_min, x_max, y_min, y_max, xMinInt, xMaxInt, yMinInt,
                         yMaxInt, xMinFrac, xMaxFrac, yMinFrac, yMaxFrac);

    if (normalize) {
      area = gpu::computeArea(x_min, x_max, y_min, y_max, exact);
    }
  } else {
    cpu::splitParameters(x_min, x_max, y_min, y_max, xMinInt, xMaxInt, yMinInt,
                         yMaxInt, xMinFrac, xMaxFrac, yMinFrac, yMaxFrac);
  ...
  // Output will be 1 pixel smaller and have `num_filters` channels per each
  // input channel
  auto output = at::empty({batchSize, nInputPlanes, numFilters, h, w},
                          input_integrated.options());
    ...
    if (normalize) {
      if (exact) {
        gpu::boxConvUpdateOutput<true, true>(
            xMinInt, xMaxInt, yMinInt, yMaxInt, xMinFrac, xMaxFrac, yMinFrac,
            yMaxFrac, area, input_integrated, output);
      } else {
...
std::vector<at::Tensor> box_convolution_backward(
    at::Tensor input_integrated, at::Tensor x_min, at::Tensor x_max,
    at::Tensor y_min, at::Tensor y_max, at::Tensor grad_output,
    at::Tensor output, const float reparametrization_h,
    const float reparametrization_w, const bool normalize, const bool exact,
    const bool input_needs_grad, const bool x_min_needs_grad,
    const bool x_max_needs_grad, const bool y_min_needs_grad,
    const bool y_max_needs_grad) {
  grad_output = grad_output.contiguous();  // TODO support noncontiguous too
  TORCH_CHECK(grad_output.dim() == 4,
              "grad_output for box_convolution must have 4 dimensions")
  TORCH_CHECK(
  ...
  TORCH_CHECK(x_min.size(0) == x_max.size(0) and
                  ...
                  x_min.size(0) == y_max.size(0) and
                  x_min.size(0) == input_integrated.size(1),
              "all box conv parameters must have as many rows as there are "
              "input channels");
  TORCH_CHECK(x_min.size(1) == x_max.size(1) and
                  x_min.size(1) == y_min.size(1) and
                  x_min.size(1) == y_max.size(1),
  ...
  grad_output =
      grad_output.reshape({batchSize, nInputPlanes, numFilters, h, w});
  ...
  // Allocate memory for splitting x_min, x_max, y_min, y_max into integer and
  // fractional parts
  // TODO how to force is_variable(false)? How to set is_variable on an existing
  // tensor?
  auto intOptions =
  ...
  if (input_needs_grad) {
    ...
    at::Tensor tmpArray = at::empty({batchSize, nInputPlanes, numFilters, h, w},
                                    grad_output.options());
    ...
    if (grad_output_integrated.is_cuda()) {
      gpu::splitParametersUpdateGradInput(x_min, x_max, y_min, y_max, xMinInt,
                                          xMaxInt, yMinInt, yMaxInt, xMinFrac,
                                          xMaxFrac, yMinFrac, yMaxFrac);
      ...
      if (normalize) {
        ...
        if (exact) {
          gpu::boxConvUpdateGradInput<true, true>(
              xMinInt, xMaxInt, yMinInt, yMaxInt, xMinFrac, xMaxFrac, yMinFrac,
              yMaxFrac, area, grad_output_integrated, tmpArray);
        } else {
      ...
      cpu::splitParametersUpdateGradInput(x_min, x_max, y_min, y_max, xMinInt,
                                          xMaxInt, yMinInt, yMaxInt, xMinFrac,
                                          xMaxFrac, yMinFrac, yMaxFrac);
  ...
  bool paramNeedsGrad[4] = {x_min_needs_grad, x_max_needs_grad,
                            y_min_needs_grad, y_max_needs_grad};
  ...
  if (someParamNeedsGrad) {
    tmpArray =
        at::empty({batchSize, nInputPlanes, numFilters, h, w}, x_min.options());

    if (x_min.is_cuda()) {
      gpu::splitParametersAccGradParameters(x_min, x_max, y_min, y_max, xMinInt,
                                            xMaxInt, yMinInt, yMaxInt, xMinFrac,
                                            xMaxFrac, yMinFrac, yMaxFrac);
      ...
      cpu::splitParametersAccGradParameters(x_min, x_max, y_min, y_max, xMinInt,
                                            xMaxInt, yMinInt, yMaxInt, xMinFrac,
                                            xMaxFrac, yMinFrac, yMaxFrac);
    ...
    for (int paramIdx = 0; paramIdx < 4; ++paramIdx) {
      if (paramNeedsGrad[paramIdx]) {
        ...
        if (input_integrated.is_cuda()) {
          if (exact) {
            gpu::boxConvAccGradParameters<true>(
                xMinInt, xMaxInt, yMinInt, yMaxInt, xMinFrac, xMaxFrac,
                yMinFrac, yMaxFrac, input_integrated, tmpArray, paramId);
          } else {
        ...
        gradParam[paramIdx] =
            tmpArray.reshape({batchSize, nInputPlanes, numFilters, h * w})
                .sum(c10::IntArrayRef({0, 3}));
    ...
    if (normalize) {  // add the second summand
      ...
      tmpArray = tmpArray.reshape({batchSize, nInputPlanes, numFilters, h * w})
                     .sum(c10::IntArrayRef({0, 3}));
      ...
      for (int paramIdx = 0; paramIdx < 4; ++paramIdx) {
        if (paramNeedsGrad[paramIdx]) {
          ...
          const bool needXDeriv =
              paramId == Parameter::xMin or paramId == Parameter::xMax;
          const bool needYDeriv = not needXDeriv;

          if (x_min.is_cuda()) {
            area = gpu::computeArea(x_min, x_max, y_min, y_max, exact,
                                    needXDeriv, needYDeriv);
          } else {
            area = cpu::computeArea(x_min, x_max, y_min, y_max, exact,
                                    needXDeriv, needYDeriv);
          }
          ...
          const bool minus =
              paramId == Parameter::xMax or paramId == Parameter::yMax;
...
void clip_parameters(at::Tensor x_min, at::Tensor x_max, at::Tensor y_min,
                     at::Tensor y_max, const double reparametrization_h,
                     const double reparametrization_w, const double max_input_h,
                     const double max_input_w, const bool exact) {
  CHECK_CONTIGUOUS(
      x_min);  // and assume other parameter tensors have same layout

  const float minWidth = exact ? 1.001f : 2.001f;
  const float minHeight = exact ? 1.001f : 2.001f;

  if (x_min.is_cuda()) {
    gpu::clipParameters(x_min, x_max, reparametrization_h, minHeight,
                        max_input_h);
    gpu::clipParameters(y_min, y_max, reparametrization_w, minWidth,
                        max_input_w);
  } else {
    cpu::clipParameters(x_min, x_max, reparametrization_h, minHeight,
                        max_input_h);
    cpu::clipParameters(y_min, y_max, reparametrization_w, minWidth,
                        max_input_w);
  }
}
