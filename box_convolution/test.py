...
def test_integral_image(device):
    ...
    for test_idx in tqdm(range(50)):
        ...
        input_image = torch.rand(
            batch_size, in_planes, h, w, requires_grad=True, device=device
        )
...
def test_box_convolution_module(device):
    def explicit_box_kernel(x_min, x_max, y_min, y_max, normalize):
        ...
        retval = torch.ones(
            1 + 2 * h_farthest, 1 + 2 * w_farthest, device=x_min.device
        )
    ...
    def box_convolution_reference(
        ...
        kernels = [
            [explicit_box_kernel(*out_c, normalize) for out_c in zip(*in_c)]
        ...
        for in_sample, out_sample in zip(input, output):
            for in_plane_idx, in_plane_kernels in enumerate(kernels):
                for filter_idx, kernel in enumerate(in_plane_kernels):
                    filtered = conv2d_single_channel(
                        in_sample[in_plane_idx], kernel
                    )
    ...
    def box_convolution_wrapper(
        ...
        for p_before, p_after, p_name in zip(
            params_before, params_after, param_names
        ):
            if not torch.equal(p_before, p_after):
                raise ValueError(
                    ...
                    + "\n".join(
                        "%s: %s" % (n, p)
                        for n, p in zip(param_names, params_before)
                    )
                    + "\n\nAfter:\n"
                    + "\n".join(
                        "%s: %s" % (n, p)
                        for n, p in zip(param_names, params_after)
                    )
                )
    ...
    for test_idx in tqdm(range(40)):
        ...
        input_image = torch.rand(
            batch_size, in_planes, h, w, device=device, requires_grad=True
        )

        # sample boxes more or less randomly (algorithm isn't practical but is OK for gradcheck)
        x_min, x_max, y_min, y_max = (
            torch.empty(in_planes, num_filters, device=device)
            for _ in range(4)
        )
        for plane_idx in range(in_planes):
            for filter_idx in range(num_filters):
                ...
                while not box_is_valid:
                    x_min_curr = random.uniform(
                        -h + 1.05, h - (not exact) - 1.1
                    )
                    y_min_curr = random.uniform(
                        -w + 1.05, w - (not exact) - 1.1
                    )

                    # set sizes to at least 1.001 because of `_clip_parameters`'s behavior
                    x_max_curr = random.uniform(
                        x_min_curr + (not exact) + 3 * gradcheck_step + 0.001,
                        h - 1.05,
                    )
                    y_max_curr = random.uniform(
                        y_min_curr + (not exact) + 3 * gradcheck_step + 0.001,
                        w - 1.05,
                    )
                    ...
                    for value in (
                        ...
                        if (
                            abs(value - round(value)) <= gradcheck_step * 3
                        ):  # *3 for extra safety
                            box_is_valid = False
        ...
        grad_output = (
            torch.rand(batch_size, in_planes * num_filters, h, w) < 0.15
        ).to(input_image)
        ...
        if not our_grad_input.allclose(
            reference_grad_input, rtol=3e-5, atol=1e-5
        ):
            raise ValueError(
        ...
        try:
            ...
            torch.autograd.gradcheck(
                ...
                eps=gradcheck_step
                / max(reparametrization_h, reparametrization_w),
                raise_exception=True,
            )
        except Exception:
            print(
                "Test %d failed at finite difference grad check w.r.t. parameters."
                % test_idx
            )
