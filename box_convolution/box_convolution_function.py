...
class BoxConvolutionFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        h_min, h_max, w_min, w_max = reparametrize(
            h_min,
            h_max,
            w_min,
            w_max,
        ...
        output = cpp_cuda.box_convolution_forward(
            input_integrated, h_min, h_max, w_min, w_max, normalize, exact
        )
    ...
    def backward(ctx, grad_output):
        ...
        if output is None:
            output = torch.empty(
                0
            )  # to satisfy `box_convolution_backward`'s signature
        ...
        retval = cpp_cuda.box_convolution_backward(
            ...
            grad_output,
            output,
            ctx.reparametrization_h,
            ctx.reparametrization_w,
            ctx.normalize,
            ctx.exact,
