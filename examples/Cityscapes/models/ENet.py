    ...
    def __init__(self, n_classes=19, max_input_h=512, max_input_w=1024):
        ...
        nn.ModuleList.__init__(
            ...
            [
                ...
                Bottleneck(64, 64, 0.01),
                BottleneckBoxConv(
                    64, 4, h // 4, w // 4, 0.15, reparam_factor=r
                ),
                Bottleneck(64, 64, 0.01),
                BottleneckBoxConv(
                    64, 4, h // 4, w // 4, 0.15, reparam_factor=r
                ...
                Bottleneck(128, 128, 0.1),
                BottleneckBoxConv(
                    128, 4, h // 8, w // 8, 0.25, reparam_factor=r
                ),
                Bottleneck(128, 128, 0.1, asymmetric_ksize=5),
                BottleneckBoxConv(
                    128, 4, h // 8, w // 8, 0.25, reparam_factor=r
                ...
                Bottleneck(64, 64, 0.1),
                BottleneckBoxConv(
                    64, 4, h // 4, w // 4, 0.1, reparam_factor=r
                ),
                Upsampler(64, 16),
                BottleneckBoxConv(
                    16, 2, h // 2, w // 2, 0.1, reparam_factor=r
                ),
                nn.ConvTranspose2d(16, n_classes + 1, (2, 2), (2, 2)),
            ],
        )
    ...
    def __init__(self, n_classes=19, max_input_h=512, max_input_w=1024):
        ...
        nn.ModuleList.__init__(
            ...
            [
                ...
                BottleneckBoxConv(
                    128, 4, h // 8, w // 8, 0.4, reparam_factor=r
                ),
                BottleneckBoxConv(
                    128, 4, h // 8, w // 8, 0.4, reparam_factor=r
                ),
                ...
                Upsampler(64, 16),
                BottleneckBoxConv(
                    16, 4, h // 2, w // 2, 0.05, reparam_factor=r
                ),
                nn.ConvTranspose2d(16, n_classes + 1, (2, 2), (2, 2)),
            ],
        )
...
class Upsampler(nn.Module):
    ...
    def forward(self, x, max_indices):
        ...
        x_skip_connection = F.max_unpool2d(
            x_skip_connection, max_indices, (2, 2)
        )
...
class Downsampler(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels, out_channels - in_channels, (3, 3), 2, 1, bias=False
        )
    ...
    def __init__(
        ...
        if asymmetric_ksize is None:
            ...
            main_branch += [
                ...
                nn.Conv2d(
                    bt_channels, bt_channels, (1, ksize), 1, (0, padding)
                ),
            ]
        ...
        self.output_activation = (
            nn.PReLU(out_channels) if use_prelu else nn.ReLU(True)
        )

    def forward(self, x):
        if self.downsample:
            x_skip_connection, max_indices = F.max_pool2d(
        ...
        if self.channels_to_pad > 0:
            x_skip_connection = F.pad(
                x_skip_connection, (0, 0, 0, 0, 0, self.channels_to_pad)
            )
