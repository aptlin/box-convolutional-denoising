...
try:
    ...
    box_video = cv2.VideoWriter(
        ...
        25,
        tuple(reversed(box_video_resolution)),
    )
    box_video_frame_count = 0
    video_background = (
        None  # to be defined in `main()`, sorry for globals and messy code
    )
except ImportError:
    box_video = None
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if box_video is not None:
            ...
            if box_video_frame_count % 5 == 0:
                ...
                video_background = video_background.unsqueeze(-1).repeat(
                    1, 1, 3
                )
                video_background = (
                    video_background.mul(255).round().byte().numpy()
                )

            # log boxes to the video file
            if batch_idx % 5 == 0:
                box_importances = (
                    model.conv1_1x1.weight.detach()
                    .float()
                    .abs()
                    .max(0)[0]
                    .squeeze()
                ...
                box_video.write(
                    cv2.addWeighted(
                        boxes_plot, 1.0, video_background, 0.25, 0.0
                    )
                )
